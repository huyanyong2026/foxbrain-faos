...
export function AICard({ title, summary, evidence, confidence, action }: AICardProps) {
  return <Card label="AI RECOMMENDATION" title={title} className="ai-card"><p>{summary}</p><p className="card-action">{action}</p><footer><span>{evidence}</span><b>{confidence}</b></footer></Card>;
}
export function EvidenceCard({ source, timestamp, version, confidence }: Evidence) {
  return <Card label="VERIFIED EVIDENCE" title={source}><dl><div><dt>采集时间</dt><dd>{timestamp}</dd></div><div><dt>数据版本</dt><dd>{version}</dd></div><div><dt>可信度</dt><dd>{confidence}</dd></div></dl></Card>;
}
...
export function AgentCard({ agent, status, tasks, insights }: AgentCardProps) {
  return <Card label={`AGENT · ${status.toUpperCase()}`} title={agent}><dl><div><dt>任务</dt><dd>{tasks}</dd></div><div><dt>洞察</dt><dd>{insights}</dd></div></dl></Card>;
}
...
export function DecisionCard({ problem, analysis, options, recommendation }: DecisionCardProps) {
  return <Card label="DECISION REQUIRED" title={problem}><dl><div><dt>分析</dt><dd>{analysis}</dd></div><div><dt>选项</dt><dd>{options}</dd></div></dl><p className="card-action">{recommendation}</p></Card>;
}
...
export function DataCard({ metric, value, trend, source }: DataCardProps) {
  return <Card label="CORE DATA" title={metric} className="data-card"><strong className="metric-value">{value}</strong><p className="trend">{trend}</p><footer><span>{source}</span></footer></Card>;
}
