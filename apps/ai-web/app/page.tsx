...
export default function WorkspacePage() {
  ...
  async function ask() { const response = await fetch(`${apiBaseUrl}/api/workspace/advice`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "product", query }) }); setResult(response.ok ? "已生成带 Citation 的产品建议；请人工核实价格、库存和尺码。" : "暂时无法获取建议。"); }
  async function loadBriefing() { const response = await fetch(`${apiBaseUrl}/api/store/dashboard?store=${store}`); setBriefing(response.ok ? "已加载销售摘要、商品机会、库存提醒与客户机会；请店长人工确认后安排任务。" : "暂时无法获取门店摘要或您没有该门店权限。"); }
