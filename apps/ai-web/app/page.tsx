...
export default function WorkspacePage() {
  const [query, setQuery] = useState("MONT适合什么客户？"); const [result, setResult] = useState("输入问题后获取只读建议。");
  ...
  return <main><h1>VAFOX Employee AI Workspace</h1><p>查询 · 分析 · 建议（不执行销售、下单或数据修改）</p><AICard title="Product Assistant"><input aria-label="AI 问题" value={query} onChange={e => setQuery(e.target.value)} /><button onClick={ask}>获取建议</button><p>{result}</p></AICard><ProductCard product="KAILAS MONT" /><CustomerCard customer="授权客户查询" /></main>;
}
