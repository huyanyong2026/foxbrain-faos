...
BA-V1 consumes Core facts only. Expected fact fields include `store_code`, `store_name`, `sku`, `product_name`, `brand`, `inventory_qty`, `incoming_qty`, `sales_30d`, `sales_prev_30d`, `sales_90d`, `aging_days`, `unit_cost`, `sales_amount_30d`, and `gross_margin_30d`.

## Derived objects

- Replenishment task: product, SKU, store, suggested quantity, priority, reason, source, human approval flag.
- Overstock task: product, SKU, store, inventory value, aging days, recommendation, reason, source, human approval flag.
- Inventory health: store, numeric score, low-stock count, overstock count.
- Briefing: what happened, why, recommended actions, evidence source.
