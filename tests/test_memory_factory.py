...
class FakeMemoryService:
    ...
    def create(self, name, content, content_type, source, owner, metadata):
        memory_id = "memory-1"; self.items[memory_id] = {"id": memory_id, "name": name, "type": content_type, "size": len(content), "source": source, "owner": owner, "metadata": metadata, "storage_path": "memory/memory-1/" + name, "status": "active", "content": content}; return {"memory_id": memory_id, "storage_path": self.items[memory_id]["storage_path"]}
    ...
    def search(self, query, owner=None):
        return [self.get(key) for key, item in self.items.items() if self.get(key) and query.lower() in (item["name"] + json.dumps(item["metadata"])).lower() and (not owner or item["owner"] == owner)]
