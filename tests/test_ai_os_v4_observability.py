...
from foxbrain_os.platform_governance import control_tower_status, version_payload


def test_version_payload_exposes_runtime_metadata(monkeypatch, tmp_path):
    metadata = tmp_path / "deployment.json"
    metadata.write_text(json.dumps({"system": "FoxBrain", "version": "AI-OS-V4.0", "commit": "abc123", "build_time": "2026-07-16T00:00:00+00:00", "deploy_time": "2026-07-16T00:01:00+00:00", "environment": "production"}))
...
def test_control_tower_has_health_dashboard_fields(monkeypatch):
    monkeypatch.setenv("GIT_COMMIT", "abc123")
