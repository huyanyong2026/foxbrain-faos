...
RBAC is represented by permissions and ABAC by `data_scope` plus `store_codes`. Every query that can expose operating facts must call `require_permission` or equivalent route middleware before returning data. Store-scoped users can only see stores in their scope.

## Audit

Permission decisions produce `AuditEvent` records containing actor, action, resource, decision, reason, and timestamp. Production routes must persist these events into the existing audit log table.
