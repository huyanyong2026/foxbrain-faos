...
业务和 API 使用逻辑 collection 名称 **`memory_chunks_v1`**；在 Qdrant 中它实现为 alias，原子指向当前已验收的不可变物理 collection。这样满足稳定的 Phase 1B collection 契约，同时避免模型切换时混用维度。切换时只重定向 `memory_chunks_v1`，不覆盖旧 collection。collection 参数建议为 `vectors.size = profile.dimension`、`distance = Cosine`，并在容量验证后选择 HNSW 索引参数。所有写入使用确定性 point id（由 `chunk_id + embedding_profile_id` 生成 UUIDv5），因此 retry/upsert 是幂等的。
...
Dify Knowledge / RAG
        ...
        ▼
Memory Factory Retrieval API ──► Qdrant memory_chunks_v1
        │                                │
...
以下工作包是后续开发的拆分和验收边界，不是本次 Phase 1B 的部署指令。它们必须在隔离开发/测试环境中完成，并保持当前 `docker-compose.yml`、Dify、SAP、Core 和生产基础设施不变。

| 工作包 | 建议职责/模块 | 主要交付物 | 完成条件 |
| --- | --- | --- | --- |
| WP1：契约与配置 | `embedding_profiles` registry、`EmbeddingProvider` protocol、BGE-M3/OpenAI/Jina adapters | 版本化 profile 配置、capabilities 校验、统一错误码和 secrets 注入约定 | 同一 fixture 的输入顺序、dimension、失败项和 idempotency 行为可由三个 adapter 一致表达。 |
| WP2：持久化与事件 | 独立 Phase 1B PostgreSQL migration、outbox publisher/consumer | 第 4.2 节五张表、索引、状态转换和死信查询 | 上传成功、更新、删除分别产生可重放且唯一的事件；不修改 Phase 1A 表字段或已有 API 语义。 |
| WP3：提取与切片 | MIME router、受限 extractor、normalizer、tokenizer-aware chunker | 提取结果/sidecar、稳定 chunk id、边界和 hash ledger | 对相同对象/revision 重跑得到相同 chunk sequence；失败有结构化错误且不会产生空向量。 |
| WP4：索引 worker | batcher、embedding retry、Qdrant repository、reconciliation job | 幂等 point upsert/delete、`memory_chunks_v1` alias 管理、lag/失败指标 | 重复事件不新增 point；删除先阻断读取；ledger 与 Qdrant 差异可检测并修复。 |
| WP5：检索服务 | query embedding、授权 filter builder、citation loader、related-memory aggregator | 第 5 节两个外部 API 和一个内部 `/embed` 契约 | owner/tag/source/time filter 均在 Qdrant 服务端生效；越权、profile 不匹配和依赖故障按规定状态码失败。 |
| WP6：只读 Dify adapter | service-identity client、Dify context mapper、audit boundary | 上下文块及 citation metadata 映射 | adapter 只调用 Retrieval API；测试证明没有 Dify/Qdrant 写请求，也不修改 Dify 配置。 |

建议实施顺序为 WP1 → WP2 → WP3 → WP4 → WP5 → WP6。WP4 不得在 WP2 的 outbox、状态机和删除阻断测试完成前接入真实 Qdrant；WP6 不得在 WP5 的授权负向测试通过前接入任何 Dify 环境。

### 7.1 接口边界与配置清单

| 边界 | 允许的输入 | 允许的输出 | 明确禁止 |
| --- | --- | --- | --- |
| Memory API → outbox | `memory_id`、revision、object hash、事件类型 | 可重放 index request | 在 HTTP handler 内同步提取、embedding 或 Qdrant 写入。 |
| worker → provider | 经过长度限制的 chunk/query 文本、profile id、idempotency key | model identity、dimension、vectors、usage/error | 将 provider key、完整文本或 vector 写到普通日志。 |
| worker → Qdrant | point id、vector、白名单 payload | upsert/delete/search response | 无 profile 校验的 collection 写入，或把正文/任意 metadata 放进 payload。 |
| Retrieval API → caller | authenticated principal、query、收窄 filters | citation-ready results | 信任客户端传入 owner，或返回未经 ledger active/ACL 校验的结果。 |
| Dify adapter → Retrieval API | service identity、已映射的授权上下文、query | context blocks/citations | 访问 PostgreSQL、MinIO 或 Qdrant 写接口，或修改 Dify dataset/workflow/provider。 |

后续实现所需的最小配置键应为：`EMBEDDING_PROFILE_REGISTRY`、各 provider 的 endpoint/credential secret reference、`QDRANT_URL`、`QDRANT_COLLECTION_ALIAS=memory_chunks_v1`、`MEMORY_INDEX_MAX_ATTEMPTS`、`MEMORY_CHUNK_TARGET_TOKENS`、`MEMORY_CHUNK_OVERLAP_TOKENS` 以及 extractor 的资源上限。配置只能在部署时注入；仓库不得提交真实 endpoint、token 或密钥。

## 8. 部署建议（仅供后续 staging 计划）
